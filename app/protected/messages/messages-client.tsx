...
export default function MessagesPageClient({
  ...
  const [selectedExchange, setSelectedExchange] = useState<ExchangeWithUI | null>(null);
    ...
    const fetchExchanges = async () => {
      ...
      if (data) {
        const formatted: ExchangeWithUI[] = data.map((ex: any) => ({
          ...
          hasUnread: ex.messages?.some((m: any) => m.sender_id !== userId && !m.is_read)
        }));
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      if (updated && updated.length > 0) {
        ...
        setExchanges(prev => prev.map(ex => ex.id === exchangeId ? { ...ex, hasUnread: false } : ex));
      }
    };
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !selectedExchange || selectedExchange.status === "completed") return;
    ...
    <main className="h-screen flex flex-col bg-[#f9f7f2]">
        ...
        <div className="flex items-center gap-10">
          <div className="flex gap-8 text-[#114c5f]">
            <Link href="/protected/swap" className="relative">
              <Repeat size={26} className="hover:text-[#0057cc] transition-all hover:scale-110" />
      ...
      <div className="flex-1 flex flex-col max-w-6xl mx-auto w-full overflow-hidden p-4">
        <div className="flex justify-end mb-2">
          <Link href="/protected/" className="text-[#0057cc] hover:text-[#004499] text-sm font-semibold">
        ...
        <div className="flex-1 flex bg-white rounded-2xl shadow-xl overflow-hidden mt-4 border border-gray-100">
          ...
          <aside className="w-80 border-r flex flex-col bg-gray-50/30">
            <div className="p-6 border-b bg-white"><h2 className="font-black text-[#1a1a1a] text-xl">Chats</h2></div>
            <div className="flex-1 overflow-y-auto">
              {exchanges.map((ex) => (
                <div key={ex.id} onClick={() => setSelectedExchange(ex)}
                  className={`p-4 flex items-center cursor-pointer border-b transition-all ${
                    selectedExchange?.id === ex.id ? "bg-[#114c5f]/10 border-l-4 border-l-[#114c5f]" : "hover:bg-white"
                  } ${ex.status === "completed" ? "opacity-60 bg-gray-100" : ""}`}
                >
                  <div className="relative flex-shrink-0">
                    <Image src={ex.contact?.avatar_url || `https://ui-avatars.com/api/?name=${ex.contact?.full_name}`}
                      alt="Avatar" width={48} height={48} className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm" />
                  ...
                  <div className="ml-4 overflow-hidden">
                    <p className="font-bold text-gray-900 truncate text-sm">{ex.contact?.full_name}</p>
                    <p className="text-[10px] text-blue-600 font-black uppercase tracking-wider">{ex.contact?.career}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {selectedExchange ? (
              <>
                <header className="p-4 border-b bg-white flex items-center shadow-sm">
                  <Image src={selectedExchange.contact?.avatar_url || `https://ui-avatars.com/api/?name=${selectedExchange.contact?.full_name}`} 
                    alt="" width={40} height={40} className="w-10 h-10 rounded-full mr-3 border" />
                  <p className="font-bold text-[#114c5f]">{selectedExchange.contact?.full_name}</p>
                </header>
                <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-4">
                  {messages.map((m) => (
                    <div key={m.id} className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${
                      m.sender_id === userId ? "self-end bg-[#114c5f] text-white rounded-tr-none" : "self-start bg-white border text-[#0d3b4c] font-bold rounded-tl-none"
                    }`}>{m.content}</div>
                  ))}
                ...
                <footer className="p-4 bg-white border-t border-gray-100">
                  <form onSubmit={handleSend} className="flex gap-2">
                    <input className="flex-1 p-3 bg-gray-50 border rounded-xl outline-none focus:ring-2 focus:ring-[#114c5f]/10 text-sm text-[#0d3b4c]"
                      value={newMessage} onChange={(e) => setNewMessage(e.target.value)}
                      disabled={selectedExchange.status === "completed"}
                      placeholder={selectedExchange.status === "completed" ? "Intercambio finalizado" : "Escribe un mensaje..."} />
                    <button type="submit" className="bg-[#114c5f] text-white px-6 py-2 rounded-xl font-bold hover:bg-[#0d3b4c] disabled:opacity-50">Enviar</button>
                  </form>
                </footer>
              </>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-gray-400 gap-4 opacity-20"><MessageSquare size={80} /><p className="font-bold text-xl">SkillSwap Chat</p></div>
            )}
          </section>
        </div>
      </div>
